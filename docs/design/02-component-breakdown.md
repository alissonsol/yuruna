...
`stash-service`, `ubuntu.server`) x five files each -- `<family>.base.user-data`,
...
```mermaid
flowchart TD
    runner[Start-TestRunner.ps1 +<br/>Invoke-TestCycleRunner.ps1]
...
| Box | Real paths | Responsibility |
|---|---|---|
| `runner` | `test/Start-TestRunner.ps1`, `test/modules/Invoke-TestCycleRunner.ps1` | The forever-living outer entry point and the fresh per-cycle child it spawns and polls. |
...
| `status` | `test/service/Start-StatusService.ps1`, `test/status/` | The `HttpListener` dashboard on port 8080, its five HTML pages, and the `runtime/` + `log/` state trees it serves. |
...
| `admin` | `test/service/` (14), `test/pool/` (13), `test/lab/` (9), `test/check/` (2) | Operator CLIs: service-VM and daemon lifecycle pairs, pool intent mutation, lab creation and joining, OCR probes. |

**Three processes, not two.** `Start-TestRunner.ps1` is a thin outer entry
...
`test/service/` (14 scripts: start/stop pairs for the status service, the
config service and the four service VMs, and the two cache-VM utilities
...
`Test-WinRtOcr.ps1`). Of the seven top-level `test/*.ps1`, six are not drawn:
the one-shot developer entry points `Invoke-TestProject.ps1` and
`Debug-TestSequence.ps1`, the validators `Test-Config.ps1` and
