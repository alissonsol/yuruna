...
Derived from `test/Start-TestRunner.ps1`, `test/service/Start-StatusService.ps1`,
...
| `runner` | outer loop, one cycle-child and one inner process per cycle, plus the detached `Invoke-PoolStorageDrain.ps1`, `Invoke-PoolPushForwarder.ps1` and `Invoke-HostAddressBeacon.ps1` sidecars | `test/Start-TestRunner.ps1` |
| `statussrv` | detached `HttpListener` on `statusService.port` (8080); PID in `runtime/server.pid` | `test/service/Start-StatusService.ps1` |
