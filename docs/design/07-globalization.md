...
Globalization infrastructure exists, but it does not make every product surface localized. The current [`locale-manifest.json`](../../globalization/locale-manifest.json) marks only `en-US` as `supported`. `pt-BR` is `planned`: its CLDR 46 Portuguese cardinal plural rule (`pt-cardinal-cldr46`) is pinned in the manifest, but no complete, independently reviewed translation return has enabled it yet. [`Test-LocaleSupport.ps1`](../../tools/Test-LocaleSupport.ps1) verifies that every locale the manifest declares `supported` has complete compiled PowerShell, browser, and Go artifacts bound to current inputs; a `planned` or `preview` locale is not held to that bar. `qps-Ploc` and `qps-Plocm` are test-only expanded and mirrored locales, not public release languages.

Source catalogs now span 15 English-owned domains under [`globalization/catalogs/en-US/`](../../globalization/catalogs/en-US/): one per Go service (`cache`, `parser`, `download`, `aggregator`, `pool`, `stash`), the shared `status` and `auth` domains each service also embeds, and the operator/CLI-facing `automation`, `host`, `runner`, `configsync`, `remediation`, `exceptions`, and `notification` domains. The compiled inventory in [`catalog-set.json`](../../globalization/manifests/catalog-set.json) currently totals 6,046 source messages, not the earlier two-catalog/26-message pilot this document once described. Their compiled outputs cover three runtimes. [`Invoke-CatalogTriage.ps1`](../../tools/Invoke-CatalogTriage.ps1) gives every domain-inventory candidate literal a disposition -- convert, internal, or unexplained -- so the remaining scan count is actionable rather than a raw guess dominated by runner/host-driver log lines. Actual catalog use now spans status, pool, cache, parser, download-agent, stash, and pool-aggregator service surfaces plus operator-facing PowerShell automation; it does not cover every label on those pages. Project test-set labels travel separately in localized metadata maps, as shown below. See the scoped source boundaries in [`conversion-authority.json`](../../globalization/manifests/conversion-authority.json) and the wider candidate census in [`domain-inventory.json`](../../globalization/manifests/domain-inventory.json).
...
[`Invoke-CatalogEmbed.ps1`](../../tools/Invoke-CatalogEmbed.ps1) then places the kernel, locale authority, provenance, and English status table into [`yuruna.common.js`](../../test/status/yuruna.common.js) and the SDK's [`yuruna.core.js`](../../test/extension/extension-sdk/webui/assets/yuruna.core.js). Each of the six Go services (caching-proxy, caching-proxy-parser, download-agent, pool-aggregator, pool-control, stash) embeds only its own domain's English table into its own `common.js`/web asset. Generated Go tables live under each service's own `internal/catalog/` package rather than one shared module; pseudo browser catalogs remain separate assets.
...
| Go | JSON decode at catalog registration; each service registers once through `sync.Once` via its own `localization.go` | [`catalog.go`](../../test/extension/extension-sdk/i18n/catalog.go) walks resident tables |
...
Full wording conversion, reviewed Portuguese catalogs and project content, and promotion of `pt-BR` remain outside the current implemented seed; the plural rule is pinned but a complete reviewed translation return is not. The generic user-preference resolver input is not a live account-preference feature. Compatibility source checks and timing markers do not substitute for native-device observations or controlled measurements.
