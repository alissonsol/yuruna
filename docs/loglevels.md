...
1. **Command-line override** -- `-logLevel Verbose` on
   ...
   `Debug-TestSequence.ps1` / `install/setup.ps1`.
...
Only the test runner (`Start-TestRunner.ps1`) and `install/setup.ps1`
...
sub-processes, `Debug-TestSequence` standalone) inherit `$env:YURUNA_LOG_LEVEL`
