...
| D2 | Canonical tag scheme (Item 2). Three live dialects: tag `2026.05.29` (no `v`), tag `v2026.05.22` (with `v`), installer `.VERSION 2026.07.07` | Bare CalVer `YYYY.MM.DD` — already matches the newest tag AND the installer's own `.VERSION`, minimizing churn. Pin URLs/clones to the tag **and record the tag→commit SHA** in the manifest | Tag naming is project policy; do not pick unilaterally |
