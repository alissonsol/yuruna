...
- Using UTM on macOS? Nested virtualization (required for Docker Desktop inside the VM) needs macOS 15 Sequoia+, Apple M3+ chip, UTM v4.6+, and the Apple Virtualization backend (not QEMU). See [UTM setup details](../vde/host.macos.utm/guest.ubuntu.desktop/read.more.md).
