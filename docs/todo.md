...
- Get to at most one "framework incident" every 24 hours
  ...
  - Step [6/13] passwdPrompt: "Current password' | 'urrent password:" - Current password: (sequence: start.guest.amazon.linux)
- Key input issues to be tested. Fix needs confirmation to unblock
  `New-RandomPassword` to include uppercase + symbols
  (`test/extension/authentication/default.psm1`)
