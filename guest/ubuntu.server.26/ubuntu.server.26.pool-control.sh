...
# 'pool-control-admin' account; fall back to whoever invoked the script (e.g.
...
if id -u pool-control-admin >/dev/null 2>&1; then
  SERVICE_USER=pool-control-admin
else
...
if ! command -v pwsh >/dev/null 2>&1; then
  ...
  if [[ ! "$PS_TAG" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    echo "pool-control: PowerShell version discovery failed (got: '$PS_TAG')" >&2
    exit 1
  fi
  PS_VER="${PS_TAG#v}"
...
if ! command -v pwsh >/dev/null 2>&1; then
  echo "pool-control: pwsh is NOT installed; the daemon cannot run any pool-admin CLI. Aborting." >&2
  exit 1
fi
...
sudo pwsh -NoProfile -NonInteractive -Command "if (-not (Get-Module -ListAvailable powershell-yaml)) { Install-Module powershell-yaml -Scope AllUsers -Force -AcceptLicense }" \
  || echo "pool-control: powershell-yaml install failed; the pool-admin CLIs will fail to parse intent. See /diagnostics." >&2
...
locate_repo_dir() {
  ...
  for enlistment in "${candidates[@]}"; do
    if [ -f "$enlistment/test/extension/pool-control/server/go.mod" ]; then
      printf '%s' "$enlistment"
      return 0
    fi
  done
...
REPO_DIR="$(locate_repo_dir)" || {
  echo "pool-control: could not locate test/extension/pool-control/server/go.mod under any /home/*/yuruna." >&2
...
# missing module through the caching proxy. Retry: a cache miss that the proxy
# cannot relay surfaces as a transient fetch failure, and one retry usually
# lands once the proxy has the object.
attempts=3
delay=10
for try in $(seq 1 "$attempts"); do
  if ( cd "$BUILD" && go build -ldflags "-X main.version=$VERSION_STR" -o pool-control . ); then
    break
  fi
...
if [[ -n "$POOL_NAS_UNC" ]]; then
  ...
  if ! mountpoint -q "$MOUNT"; then
    sudo timeout 60 mount "$MOUNT" || echo "pool-control: NAS mount failed; persistence disabled" >&2
  fi
fi
...
# A pool-control VM with no intent store has no working UI: every read and write
...
# Seeding matches the caching-proxy's own bootstrap byte for byte -- an empty,
...
if [[ -n "$INTENT_GIT_URL" && "$INTENT_GIT_URL" != *://* && ! -d "$INTENT_GIT_URL/refs" ]]; then
  ...
  if [[ "$INTENT_GIT_URL" == "$MOUNT"/* ]] && ! mountpoint -q "$MOUNT" 2>/dev/null; then
    echo "pool-control: $MOUNT is not mounted; refusing to create $INTENT_GIT_URL on local disk (a later mount would shadow it)." >&2
    INTENT_GIT_URL=''
  else
    ...
    if sudo -u "$SERVICE_USER" git init --bare --initial-branch=main "$INTENT_GIT_URL" >/dev/null 2>&1 &&
      ...
      if sudo -u "$SERVICE_USER" git -C "$SEED_TMP" push -q "$INTENT_GIT_URL" HEAD:main; then
        ...
        echo "pool-control: intent store seeded at $INTENT_GIT_URL (empty, schema-valid)"
      else
        echo "pool-control: could not seed $INTENT_GIT_URL; the UI will report an unreadable store." >&2
      fi
      sudo -u "$SERVICE_USER" rm -rf "$SEED_TMP"
    else
      echo "pool-control: could not create $INTENT_GIT_URL; the UI will report an unreadable store." >&2
    fi
  fi
fi
if [[ -n "$INTENT_GIT_URL" && "$INTENT_GIT_URL" != *://* ]]; then
  ...
  if [[ -d "$INTENT_GIT_URL/refs" ]]; then
    echo "pool-control: intent store -> $INTENT_GIT_URL (writable)"
  fi
fi
if [[ -z "$INTENT_GIT_URL" ]]; then
  echo "pool-control: no intent store resolved (pool NAS unmounted and no seeded URL); the UI will start read-only and report why on /diagnostics." >&2
fi
...
Documentation=https://yuruna.link/pool-control
...
ExecStart=/usr/local/bin/pool-control --http-addr=\${POOL_CONTROL_HTTP_ADDR} --repo-dir=\${POOL_CONTROL_REPO_DIR} --pwsh=\${POOL_CONTROL_PWSH} --aggregator-url=\${POOL_CONTROL_AGGREGATOR_URL} --host-id=\${POOL_CONTROL_HOST_ID} --intent-git-url=\${POOL_CONTROL_INTENT_GIT_URL} --state-dir=\${POOL_CONTROL_STATE_DIR} --presence-interval=\${POOL_CONTROL_PRESENCE_INTERVAL}
Restart=on-failure
RestartSec=5
...
for _ in 1 2 3 4 5 6; do
  if sudo systemctl is-active --quiet pool-control.service; then
    break
  fi
  sleep 1
done
