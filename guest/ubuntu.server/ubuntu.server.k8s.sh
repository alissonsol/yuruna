...
[host."http://${CACHE_HOST}:5000"]
  capabilities = ["pull", "resolve"]
HOSTSEOF
# public.ecr.aws was the root cause of the 2026-05-13 incident where
# `registry:2` started returning 400. Mirroring it via zot means the
# next time AWS ECR Public has an image-specific hiccup, zot serves
# the cached copy instead of fronting the upstream's failure.
...
# github.com/FiloSottile/mkcert releases). That endpoint returned a transient
# HTTP 500 in cycle 000437 (2026-05-15) and aborted the whole k8s.website
# sequence under set -euo pipefail. Retry 3x with backoff, then fall back to
# Ubuntu universe's mkcert package, and only exit if both paths fail.
echo ""
