...
The `guest.<service>-service/` folders (stash, pool-control, download-agent)
hold the same three files on every host and differ only in base image and the
hypervisor call -- see
[What each service VM folder holds](../docs/guest-image-setup.md#what-each-service-vm-folder-holds).
...
6. Run `pwsh test/Start-TestRunner.ps1`.
