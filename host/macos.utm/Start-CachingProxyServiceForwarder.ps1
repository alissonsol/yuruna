...
<#
.SYNOPSIS
    Cross-platform userspace TCP forwarder for the caching-proxy-service VM.
...
.DESCRIPTION
    ...
    Typically launched detached by Start-CachingProxyServiceVM.ps1 (via
    Yuruna.Host.psm1's Start-CachingProxyServiceForwarder) and killed by
    Stop-CachingProxyServiceVM.ps1. PID is written to -PidFile so the stopper can
    find it without pgrep.

.PARAMETER CacheIp
    IP of the caching-proxy-service VM on the VZ shared-NAT subnet (typically
...
.EXAMPLE
    pwsh Start-CachingProxyServiceForwarder.ps1 -CacheIp 192.168.64.3

.EXAMPLE
    # SSH on a non-standard host port:
    pwsh Start-CachingProxyServiceForwarder.ps1 -CacheIp 192.168.64.3 -Port 8022 -VMPort 22

.EXAMPLE
    # How Start-CachingProxyServiceVM.ps1 launches it (detached):
    Start-Process pwsh -ArgumentList @(
        '-NoProfile','-File', $forwarderScript,
        '-CacheIp', '192.168.64.3',
        '-PidFile', "$HOME/yuruna/image/caching-proxy-service/forwarder.pid",
        '-LogFile', "$HOME/yuruna/image/caching-proxy-service/forwarder.log"
    )
#>

param(
    # Default port honors $env:YURUNA_CACHING_PROXY_SERVICE_HTTP_PORT (3128 fallback).
    # This script runs in a fresh pwsh subprocess (Start-Process pwsh -File),
    # so the Get-CachingProxyServicePort helper is not yet imported -- read the env
    # var directly to keep the same defaulting behavior callers expect.
    [Parameter(Mandatory)][string]$CacheIp,
    [int]$Port = $(if ($env:YURUNA_CACHING_PROXY_SERVICE_HTTP_PORT -as [int]) { [int]$env:YURUNA_CACHING_PROXY_SERVICE_HTTP_PORT } else { 3128 }),
    [int]$VMPort = 0,
