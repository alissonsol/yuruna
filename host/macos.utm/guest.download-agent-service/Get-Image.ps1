<#PSScriptInfo
.VERSION 2026.08.25
.GUID 42f387ed-14a2-486a-ac4d-fc01b6374170
...
<#
.SYNOPSIS
    Provides the Ubuntu server cloud image that backs the download-agent
...
.DESCRIPTION
    ...
    stays so the download-agent service can move to a different release,
    arch or post-processing step later without disturbing the others.
...
if (-not $IsMacOS) {
    Write-Error "host/macos.utm/guest.download-agent-service/Get-Image.ps1 only runs on macOS."
    exit 1
}
