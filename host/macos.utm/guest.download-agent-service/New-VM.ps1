...
# from the network the VM lands on -- so Resolve-UtmNetworkMode decides once here
# and the config.plist below is rendered from that same value.
...
if ($NetworkMode -eq 'Shared') {
    Write-Output "Default route is Wi-Fi ($BridgeInterface) -- bridged can't get a LAN lease over Wi-Fi; building the download-agent-service VM on UTM Shared NAT. Start-DownloadAgentServiceVM.ps1 will forward a host port to it for LAN access."
} else {
...
# the stash-service and pool-control-service VMs. UTM's MemorySize is a fixed
...
See https://yuruna.link/download-agent-service.
'@
