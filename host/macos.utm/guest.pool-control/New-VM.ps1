...
<#
.SYNOPSIS
    Builds the Yuruna Pool control VM bundle for macOS UTM.
...
.DESCRIPTION
    ...
    and runs the bring-up script which builds the pool-control daemon,
...
.PARAMETER VMName
    Name of the UTM VM. Default: yuruna-pool-control.
#>
...
$baseImageName = "host.macos.utm.guest.pool-control"
...
# coordinates (the NAS), baked into the seed. Topology-aware host address:
...
$UserData = New-CloudInitUserData `
    -BasePath    (Join-Path $_repoRoot 'host/vmconfig/pool-control.base.user-data') `
    -OverlayPath (Join-Path $_repoRoot 'host/vmconfig/pool-control.utm.overlay.yml') `
    -RepoRoot    $_repoRoot `
    -Replacement @{
...
Write-Output "Bridge interface: $BridgeInterface (pool-control VM will request DHCP on this LAN)"
...
# --- REGION: Guidance
Write-Output ""
Write-Output "== pool-control VM bundle created =="
...
See https://yuruna.link/pool-control.
'@
