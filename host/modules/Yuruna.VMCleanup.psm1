...
.DESCRIPTION
    Each of the three platform scripts ([host/windows.hyper-v/Remove-OrphanedVMFiles.ps1](../host/windows.hyper-v/Remove-OrphanedVMFiles.ps1),
    ...
    used to hand-roll the same Write-Status routing function and (on
    Hyper-V + UTM) the same base-image-name discovery loop. The Ubuntu
    copy diverged in a small way too -- it used `$Quiet` directly where
    Hyper-V and UTM used `$script:QuietOutput`, so a future tweak to the
    quiet contract would have missed one of the three. Both helpers live
    here now so the routing + naming stay in lockstep.
#>
