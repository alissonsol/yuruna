...
.DESCRIPTION
    Same shape as guest.ubuntu.server/New-VM.ps1 but uses the AL2023 qcow2
...
if (-not $IsLinux) {
    Write-Error "host/ubuntu.kvm/guest.amazon.linux/New-VM.ps1 only runs on Linux."
    exit 1
}
...
$baseImageName = "host.ubuntu.kvm.guest.amazon.linux"
...
# guest.ubuntu.server/New-VM.ps1 sibling for the why.
...
# `--cdrom` does on guest.ubuntu.server. Libvirt's domain default is
...
# Test-Start.guest.amazon.linux.json -- with `restart`, QEMU performs
