...
| User | `yauser1` (test sequence target, see [test/sequences/gui/start.guest.amazon.linux.yml](../../../test/sequences/gui/start.guest.amazon.linux.yml)) and `ec2-user` (cloud-image default; SSH key-auth) |
| Net  | libvirt `default` (NAT 192.168.122.0/24) |

The test user's password is sourced from the per-cycle authentication vault
...
ssh -i ../../../test/.ssh/yuruna_ed25519 yauser1@<ip>
```
