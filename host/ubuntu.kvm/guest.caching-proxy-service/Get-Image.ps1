...
.DESCRIPTION
    ...
    ~/yuruna/image/caching-proxy-service/ as the base image for the long-lived
    Squid caching-proxy-service VM. libvirt-qemu boots the qcow2 natively, so no
...
# macos.utm caching-proxy-service guests so a cache rebuilt on any host produces
# the same Squid 7.x baseline. `unattended-upgrades` (enabled in
# host/vmconfig/caching-proxy-service.base.user-data) keeps pulling security patches automatically so
...
# 65 GB` directive in host/vmconfig/caching-proxy-service.base.user-data lets the proxy cache files
