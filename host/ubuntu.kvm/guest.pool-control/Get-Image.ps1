<#PSScriptInfo
.VERSION 2026.07.22
.GUID 42a5c6d7-b8c9-4e01-2345-f6a7b8c9d0e5
...
.DESCRIPTION
    ...
    ~/yuruna/image/pool-control/ as the base image for the pool-control
...
if (-not $IsLinux) {
    Write-Error "host/ubuntu.kvm/guest.pool-control/Get-Image.ps1 only runs on Linux."
    exit 1
}

# --- REGION: Configuration
# Ubuntu 26.04 LTS (Resolute Raccoon), per the pool-control spec
# (default image ubuntu.server.26), matching the stash-service LTS so the
# VM stays in the supported-LTS window and the distro Go toolchain satisfies
# the daemon's go.mod directive without a toolchain fetch.
$arch = (& uname -m).Trim()
switch ($arch) {
