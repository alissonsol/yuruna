...
<#
.SYNOPSIS
    Creates the Yuruna Pool control VM on Ubuntu KVM (libvirt).

.DESCRIPTION
    Builds a libvirt VM that boots the Ubuntu 26.04 LTS cloud image
    for the pool-control daemon (operator UI + API for the pool intent).
    ...
    See https://yuruna.link/pool-control for the full specification.

.PARAMETER VMName
    libvirt domain name. Default: yuruna-pool-control.
#>
...
if (-not $IsLinux) {
    Write-Error "host/ubuntu.kvm/guest.pool-control/New-VM.ps1 only runs on Linux."
    exit 1
}
...
$downloadDir   = "$HOME/yuruna/image/pool-control"
$baseImageName = "host.ubuntu.kvm.guest.pool-control"
...
$baseUserData     = Join-Path $repoRoot 'host/vmconfig/pool-control.base.user-data'
$overlayUserData  = Join-Path $repoRoot 'host/vmconfig/pool-control.kvm.overlay.yml'
$metaDataTemplate = Join-Path $repoRoot 'host/vmconfig/pool-control.meta-data'
...
if ($networkName -eq 'default') {
    Write-Warning "Using libvirt NAT 'default' network (192.168.122/24). The pool-control VM is reachable from this host only and the NAS likely isn't routable; define a bridged 'yuruna-external' libvirt network for LAN + NAS access."
} else {
    Write-Output "Using libvirt network: $networkName (pool-control VM will get a LAN-routable IP)"
}

# Host coordinates (status server, for the in-VM source fetch) + pool storage
...
# Pool-aggregator base URL for the daemon's presence beacon + remote-host
