...
if (-not $IsLinux) {
    Write-Error "host/ubuntu.kvm/guest.squid-cache/New-VM.ps1 only runs on Linux."
    exit 1
}
...
# The squid-cache VM is long-lived (one job: serve the cache to every
...
if ($networkName -eq 'default') {
    ...
    Write-Warning "libvirt network -- see host/ubuntu.kvm/guest.squid-cache/README.md."
    Write-Warning ""
} else {
...
for ($i = 0; $i -lt $maxIterations; $i++) {
    ...
    # 'ipv4' column. An inline `(\d+\.\d+\.\d+\.\d+)/\d+` regex
    # previously matched the 'lo ... ipv4 127.0.0.1/8' row that
    # `--source agent` emits as its FIRST line, sending the next
    # port-wait probe to the host's own loopback and stalling
    # Start-CachingProxy.ps1 with `Cache VM IP: 127.0.0.1`.
...
$portMaxIterations = 180  # 180 * 5s = 15 minutes
...
for ($i = 0; $i -lt $portMaxIterations; $i++) {
    ...
    # Print a one-line status every ~30 s (~6 iterations of 5 s) -- see
    # the rationale on the IP-wait loop above. The probe itself takes
    # 1 s blocking on BeginConnect + 4 s Start-Sleep below = 5 s/iter.
    if (($i % 6) -eq 5) {
        ...
        $min     = [int][math]::Floor($elapsed / 60)
        $sec     = [int]($elapsed % 60)
        $totalMin = [int][math]::Floor($portMaxIterations * 5 / 60)
