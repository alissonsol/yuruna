...
# ubuntu-server/daily-live/ path is rolling and serves whichever codename
# is currently in development, so requesting resolute-live-server-<arch>.iso
# there can 404 if it has rolled past resolute.
if ($cloudArch -eq 'amd64') {
    ...
    $stableReleaseUrl = 'https://cdimage.ubuntu.com/releases/resolute/release'
    $stablePattern    = 'ubuntu-[\d.]+-live-server-arm64\.iso'
    $dailyBaseUrl     = 'https://cdimage.ubuntu.com/ubuntu-server/resolute/daily-live/current'
    $dailyIsoFileName = 'resolute-live-server-arm64.iso'
}
...
# host/macos.utm/guest.ubuntu.server.26/Get-Image.ps1's
