...
<#
.SYNOPSIS
    Creates a libvirt VM that installs Ubuntu Server 26.04 via the
    live-server ISO + subiquity autoinstall.

.DESCRIPTION
    Mirrors host/macos.utm/guest.ubuntu.server.26/New-VM.ps1 and
    host/windows.hyper-v/guest.ubuntu.server.26/New-VM.ps1 so all three
...
if (-not $IsLinux) {
    Write-Error (Format-YurunaOperatorMessage -Key 'host.operator_5cd00af4f123b6b1')
    exit 1
}
