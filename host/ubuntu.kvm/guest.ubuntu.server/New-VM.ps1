...
.DESCRIPTION
    Mirrors host/macos.utm/guest.ubuntu.server/New-VM.ps1 and
    host/windows.hyper-v/guest.ubuntu.server/New-VM.ps1 so all three
...
param(
    ...
    # sequences. Default 'yuuser1' chosen for greppability (vs the
...
if (-not $IsLinux) {
    Write-Error "host/ubuntu.kvm/guest.ubuntu.server/New-VM.ps1 only runs on Linux."
    exit 1
}
...
# script generated test/.ssh/host.ubuntu.kvm and silently broke
...
if (-not $plaintextPassword) {
    ...
    $plaintextPassword = Get-Password -Username $Username
    if (-not $plaintextPassword) { Write-Error "Get-Password returned empty for '$Username'."; exit 1 }
    Write-Output "Password came from authentication mechanism: $_authActiveName"
    Write-Output "See configuration at: $(Resolve-ExtensionAreaDir -Area 'authentication')"
} else {
    Write-Output "Password came from environment variable: YURUNA_GUEST_PASSWORD"
}
$pwHash = (& openssl passwd -6 $plaintextPassword).Trim()
if (-not $pwHash) { Write-Error "openssl passwd -6 produced empty output."; exit 1 }
...
# Fresh 32 G qcow2; subiquity will partition + install onto it.
if (Test-Path -LiteralPath $diskImg) { Remove-Item -Force -LiteralPath $diskImg }
& qemu-img create -f qcow2 $diskImg 32G | Out-Null
...
Write-Verbose "Default credentials - username: $Username, password: <vault-managed> (must be changed on first login). Vault: test/extension/authentication/vault.yml (set YURUNA_GUEST_PASSWORD to bypass vault for ad-hoc dev runs)"
