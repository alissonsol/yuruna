...
-> text-mode login at `yuuser1` / `<vault-managed>` (password expired on
...
1. Renders `vmconfig/user-data` and `vmconfig/meta-data` with hostname,
   ...
   `test/.ssh/yuruna_ed25519` if missing), password hash, optional
...
4. Defines + starts the VM via `virt-install` against `qemu:///system`,
   ...
   subiquity finishes the install completes a reboot and lands at the
...
| User | `yuuser1` / `<vault-managed>` | `-Username` / `$env:YURUNA_GUEST_PASSWORD` |
| Net  | libvirt `default` (NAT 192.168.122.0/24) | (edit script) |

The first-boot password is sourced from the per-cycle authentication
vault under [`test/extension/authentication/`](../../../test/extension/authentication/)
(see [test/read.more.md](../../../test/read.more.md) for the model)
...
ssh -i ../../../test/.ssh/yuruna_ed25519 yuuser1@<ip>
