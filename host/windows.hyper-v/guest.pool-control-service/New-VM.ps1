...
<#
.SYNOPSIS
    Creates (or recreates) the Yuruna pool-control service VM on Hyper-V.

.DESCRIPTION
    Builds an Ubuntu 26.04 LTS cloud-image VM that hosts the pool-control-service
...
if (-not $switchName) {
    ...
    Write-Verbose "  The pool-control-service VM won't be reachable from LAN by its own IP, and the NAS may be unreachable."
...
$UserData = New-CloudInitUserData `
    ...
    -OverlayPath (Join-Path $_repoRoot 'host/vmconfig/pool-control-service.hyperv.overlay.yml') `
    -RepoRoot    $_repoRoot `
    -Replacement @{
...
# Single-candidate pick: the pool-control-service UI listens on :80 but v1 does not
...
# with `ssh pool-control-service-admin@<ip>` -- the harness key is already authorized.
...
Write-Output "  Console:  vmconnect localhost $VMName  (user pool-control-service-admin, vault password)"
Write-Output ""
