...
<#
.SYNOPSIS
    Creates (or recreates) the Yuruna Pool control VM on Hyper-V.

.DESCRIPTION
    Builds an Ubuntu 26.04 LTS cloud-image VM that hosts the pool-control
    daemon (operator UI + API that drives the pool-intent git store).
...
.PARAMETER VMName
    Name of the Hyper-V VM. Default: yuruna-pool-control.
#>
...
$baseImageName = "host.windows.hyper-v.guest.pool-control"
...
$UserData = New-CloudInitUserData `
    ...
    -OverlayPath (Join-Path $_repoRoot 'host/vmconfig/pool-control.hyperv.overlay.yml') `
    -RepoRoot    $_repoRoot `
    -Replacement @{
...
# shells out to + future in-VM UI. Matches the stash VM profile so the
