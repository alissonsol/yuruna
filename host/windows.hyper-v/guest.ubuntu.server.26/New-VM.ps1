...
<#
.SYNOPSIS
    Creates a Hyper-V VM that installs Ubuntu Server 26.04 in text mode.
...
# Without it, the 26.04 guest's subiquity NetworkController can pick
# up a stream of CHANGE events from Hyper-V's MAC-mismatch filtering
# during early boot and stall the install at curtin's initial step
# even though the user-data netplan (`accept-ra: false`) suppresses
# the IPv6-RA-driven variant of the same loop. Mirrors the macos.utm
# 26.04 guest's NIC-model switch to e1000 -- same intent, different
# knob per hypervisor.
Set-VMNetworkAdapter -VMName $VMName -MacAddressSpoofing On | Out-Null
