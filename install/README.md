...
# macOS / Ubuntu  (env var or flag, equivalent)
PIN_VERSION=1 ./install/ubuntu.kvm.sh
./install/ubuntu.kvm.sh --pin-version
```
...
> (`install.sha256.sig`, `install/keys/`) first ship in release `2026.07.10`;
...
```
$base='https://raw.githubusercontent.com/alissonsol/yuruna/refs/tags/2026.07.10'; $t=Join-Path $env:TEMP 'yuruna-install'; New-Item -ItemType Directory -Force $t|Out-Null
...
```
BASE='https://raw.githubusercontent.com/alissonsol/yuruna/refs/tags/2026.07.10'; S=install/macos.utm.sh   # or install/ubuntu.kvm.sh
