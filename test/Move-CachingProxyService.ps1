...
.PARAMETER OldAddress
    IP or hostname of the OLD (source) caching-proxy-service VM.

.PARAMETER OldUser
    SSH login user on the old VM. Default: caching-proxy-service-admin, the
...
.PARAMETER NewAddress
    IP or hostname of the NEW (replacement) caching-proxy-service VM.

.PARAMETER NewUser
    SSH login user on the new VM. Default: caching-proxy-service-admin, the
...
param(
    ...
    [string]$OldUser = 'caching-proxy-service-admin',
    ...
    [ValidatePattern('^[A-Za-z0-9._][A-Za-z0-9._-]*$')]
    [string]$NewUser = 'caching-proxy-service-admin',
...
if ($usageErrors.Count -gt 0) {
    foreach ($usageError in $usageErrors) { Write-Error "Move-CachingProxyService: $usageError" }
    @(
        'Usage:'
        '  pwsh test/Move-CachingProxyService.ps1 -Start -OldAddress <old> -NewAddress <new> [-OldUser yuruna] [-OldPassword ...] [-NewUser yuruna] [-NewPassword ...]'
        '  pwsh test/Move-CachingProxyService.ps1 -End   -OldAddress <old> -NewAddress <new> [-OldUser yuruna] [-OldPassword ...] [-NewUser yuruna] [-NewPassword ...]'
...
try {
    ...
    if ($Start) {
        ...
        foreach ($pair in @(@($old, $oldFact), @($new, $newFact))) {
            ...
            if (-not $fact.SquidBin -or -not $fact.ConfD) {
                throw "$($vm.Label) ($($vm.Address)): squid or /etc/squid/conf.d not found -- this does not look like a yuruna caching-proxy-service VM."
            }
        ...
        try {
            ...
            $oldBaseLines = @(
                '# Yuruna cache-migration drop-in, managed by test/Move-CachingProxyService.ps1.'
        ...
        @(
            ...
            "       Windows:      `$Env:YURUNA_CACHING_PROXY_SERVICE_IP = '$NewAddress'"
            "       macOS/Linux:  export YURUNA_CACHING_PROXY_SERVICE_IP=$NewAddress"
            ...
            "       pwsh test/Test-CachingProxyService.ps1 -CacheIp $NewAddress"
            ...
            "       pwsh test/Move-CachingProxyService.ps1 -End -OldAddress $OldAddress -NewAddress $NewAddress"
            ''
    ...
    if ($End) {
        ...
        @(
            ...
            '(default VM name: yuruna-caching-proxy-service):'
            '  1. Power the VM off on its host:'
            '       Hyper-V:     Stop-VM -Name yuruna-caching-proxy-service'
            '       Ubuntu KVM:  virsh shutdown yuruna-caching-proxy-service'
            '       macOS UTM:   utmctl stop yuruna-caching-proxy-service'
            '  2. On that same host, tear down host-side plumbing that pointed at'
            '     it (port forwards, host-proxy promotion):'
            '       pwsh test/Stop-CachingProxyServiceVM.ps1'
    ...
    Write-Error "Move-CachingProxyService aborted: $($_.Exception.Message)" -ErrorAction Continue
    $failed = $true
} finally {
