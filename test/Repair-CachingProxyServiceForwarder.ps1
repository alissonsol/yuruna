...
<#
.SYNOPSIS
    Verify the caching-proxy-service VM is reachable on the LAN and refresh the
    yuruna-caching-proxy-service state file. macOS/UTM only.
...
.DESCRIPTION
    ...
    Stop-CachingProxyServiceVM. This script:
      ...
      1. Calls Test-CachingProxyServiceAvailable (state file only, no scan --
...
.EXAMPLE
    pwsh test/Repair-CachingProxyServiceForwarder.ps1

.EXAMPLE
    # State file has the wrong IP; you already know the right one:
    pwsh test/Repair-CachingProxyServiceForwarder.ps1 -CacheIp 192.168.7.150
#>
...
if (-not $IsMacOS) {
    Write-Error "Repair-CachingProxyServiceForwarder.ps1 is macOS-only. On Hyper-V the External vSwitch keeps the cache LAN-direct with no host-side layer to repair."
    exit 1
}
...
# CachingProxyService kind loads Test.VMUtility + Test.CachingProxyService + Test.HostContract
...
# Must come AFTER Initialize-YurunaHost so Save-/Read-CachingProxyServiceState survive:
...
$StateFile = Get-CachingProxyServiceStatePath
...
# YURUNA_CACHING_PROXY_SERVICE_IP back at the Mac instead of the cache) from
...
if ($CacheIp) {
    ...
    $probe = Invoke-CachingProxyServiceProbe -CacheIp $CacheIp -CacheSource '-CacheIp parameter'
    ...
    # Test-CachingProxyServiceAvailable does the state-file fast path + LAN
    # /24 scan + state refresh atomically. No need to re-implement here.
    $foundUrl = Test-CachingProxyServiceAvailable
    if (-not $foundUrl) {
        Write-Error "Could not locate yuruna-caching-proxy-service VM on the LAN."
        Write-Error "  Yuruna.Host.psm1's Test-CachingProxyServiceAvailable returned no URL."
        Write-Error "  If the VM is stopped/missing, rebuild with: pwsh test/Start-CachingProxyServiceVM.ps1"
        Write-Error "  If you already know the VM's LAN IP, rerun with:"
        Write-Error "    pwsh test/Repair-CachingProxyServiceForwarder.ps1 -CacheIp <lan-ip>"
        exit 1
    }
