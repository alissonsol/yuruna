...
if ($IsLinux) {
    ...
    if ($env:YURUNA_EXTERNAL_BRIDGE_SKIP -eq '1') {
        ...
        if ($plannedBridge.WillChangeHostNetworking) {
            ...
            Write-Output "  >>   YURUNA_EXTERNAL_BRIDGE_SKIP=1 ./Start-CachingProxyServiceVM.ps1        <<"
        }
    }
}
...
if ($IsMacOS) {
    ...
    if ($cacheIp) {
        ...
        if (Test-CacheVMOnExternalNetwork -VMName $VMName) {
            ...
            Write-Warning "    connect the host by Ethernet, then re-run ./Start-CachingProxyServiceVM.ps1"
