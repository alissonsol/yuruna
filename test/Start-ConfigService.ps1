...
<#
.SYNOPSIS
    config service: serves NAS connection info + credentials over mutual
...
.DESCRIPTION
    ...
    host's Config CA (Test.ConfigServiceCA.psm1) and REQUIRES a client certificate
...
Import-Module (Join-Path $ModulesDir 'Test.ConfigServiceCA.psm1') -Global -Force
...
function Invoke-YurunaConfigServeLoop {
    ...
    $listener.Start()
    Write-Information "config service listening (mTLS) on 0.0.0.0:$ListenPort" -InformationAction Continue
    try {
        while ($true) {
...
function Show-YurunaConfigServerLog {
    ...
    foreach ($entry in @(@('config-server.err', 'stderr'), @('config-server.out', 'stdout'))) {
        ...
        $shownAny = $true
        Write-Warning "  --- config service $($entry[1]) ($($entry[0])) ---"
...
if (-not $Restart -and $existingProc -and (Test-YurunaConfigPortAccepting -ProbePort $Port)) {
    Write-Output "config service already running (pid $existingPid) on :$Port; no action."
...
if ($existingProc -and (Test-PidFileIdentity -PidFile $PidFile -Process $existingProc)) {
    Write-Output "Replacing config service (pid $existingPid)..."
    ...
    Write-Warning "PID $existingPid is not the config service (started after the PID file -- recycled onto an unrelated process); leaving it running and clearing the stale PID file."
}
...
if ($IsWindows) {
    try {
        $ruleName = 'Yuruna config service'
...
if ($ready) {
    Write-Output "config service up on :$Port (mTLS; serves NAS creds to this host's VMs)."
} else {
    Write-Warning "config service process started but port $Port is not accepting after $script:ConfigServiceReadyTimeoutSeconds s."
