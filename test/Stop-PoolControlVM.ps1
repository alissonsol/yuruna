...
.DESCRIPTION
    Symmetric with Start-PoolControlVM. Removes runtime/pool-control.json (so
    Test.Capability drops the pool-control area) and refreshes the registration
    record so the host clears from the dashboard's Extension hosts table. Then
    tears down the pool-control VM and every file it owns -- the registry/domain
...
if (Test-Path -LiteralPath $marker) {
    try {
        ...
        if ($m.pid -and $PSCmdlet.ShouldProcess("pid $($m.pid)", 'Stop host-side pool-control')) {
            Stop-Process -Id ([int]$m.pid) -Force -ErrorAction SilentlyContinue
        }
    } catch { Write-Verbose "stop host-side pool-control: $($_.Exception.Message)" }
    Remove-Item -LiteralPath $marker -Force -ErrorAction SilentlyContinue
    Write-Information "  Cleared pool-control marker (host will drop from Extension hosts)." -InformationAction Continue
}

# Publish the removal NOW: regenerate host.registration.json so the marker's
# absence (activeExtensions drops 'pool-control') reaches the aggregator on its
...
# Tear down the pool-control VM and every file it owns so the next Start rebuilds
...
if ($finalState -eq 'absent') {
    Write-Information "Pool control stopped; marker cleared; VM '$VMName' and its files removed." -InformationAction Continue
    exit $ExitOk
}
