...
if ($Uninstall) {
    # Uninstall isn't in the contract -- it's a manual maintenance op.
    # Stop first, then warn that capability removal is operator-driven.
    $ok = Stop-SshServer -Confirm:$false
    if (-not $ok) {
        Write-Error "SSH server uninstall failed. See warnings above."
        exit 1
    }
    Write-Output ""
    Write-Output "SSH server: stopped. Capability removal is now an operator step:"
    Write-Output "  Get-WindowsCapability -Online -Name 'OpenSSH.Server*' | Remove-WindowsCapability -Online"
} else {
    $ok = Stop-SshServer -Confirm:$false
    if (-not $ok) {
