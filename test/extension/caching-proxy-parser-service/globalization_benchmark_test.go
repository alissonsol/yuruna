...
func BenchmarkGlobalizationLookup(b *testing.B) {
	...
	if value := catalog.Render("parser.squid_cache_recent_100_requests", nil, locale); value == "parser.squid_cache_recent_100_requests" || len(catalog.MissingKeys()) != 0 {
		b.Fatal("benchmark requires the delivered locale catalog")
	}
	b.ReportAllocs()
	b.ResetTimer()
	for index := 0; index < b.N; index++ {
		if catalog.Render("parser.squid_cache_recent_100_requests", nil, locale) == "" {
