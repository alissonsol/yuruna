...
func TestDownloadVerifyPromoteCommitRoundTrip(t *testing.T) {
	...
	a := newTestAgent(t, Options{PoolDir: t.TempDir(), AgentVersion: "2026.08.19"})
...
func TestRefreshRecordsTheProbedSizeAndTheResolvedVariant(t *testing.T) {
	...
	a := newTestAgent(t, Options{PoolDir: t.TempDir(), Transport: ubuntuFixture(t, body, false, 0), AgentVersion: "2026.08.19"})
