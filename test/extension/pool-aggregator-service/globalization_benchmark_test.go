...
func BenchmarkGlobalizationLookup(b *testing.B) {
	...
	if value := catalog.Render("aggregator.no_live_host_for_that_extension_area", nil, locale); value == "aggregator.no_live_host_for_that_extension_area" || len(catalog.MissingKeys()) != 0 {
		b.Fatal("benchmark requires the delivered locale catalog")
	}
	b.ReportAllocs()
	b.ResetTimer()
	for index := 0; index < b.N; index++ {
		if catalog.Render("aggregator.no_live_host_for_that_extension_area", nil, locale) == "" {
...
func BenchmarkGlobalizationConcurrentRender(b *testing.B) {
	...
	b.RunParallel(func(pb *testing.PB) {
		...
		for pb.Next() {
			...
			localizedHTTPError(response, request, "aggregator.no_live_host_for_that_extension_area", http.StatusBadGateway)
			if response.Body.Len() == 0 || response.Header().Get("Content-Language") != locales[index%len(locales)] {
