...
func TestRedactEventLine(t *testing.T) {
	in := `{"cycleNumber":658,"hostname":"Alius202605a","event":"cycle_start","hostId":"4253419c","cycleStartUtc":"2026-06-10T15:44:32Z","cycleFolder":"000658.2026-06-10.15-44-31.Alius202605a","runId":"ce338d4c"}`
	...
	for _, k := range []string{"cycleNumber", "event", "hostId", "cycleStartUtc", "runId"} {
		if _, ok := m[k]; !ok {
...
func TestHostInfoCommitLabels(t *testing.T) {
	...
	hv := &hostView{HostId: "4253419c", BaseURL: "http://192.168.7.13:8080", Reachable: true, Version: "2026.08.04", PoolId: "lab", PoolGuid: "42a1b2c3-d4e5-4f60-8a1b-2c3d4e5f6071"}
...
func TestHostViewJSONHostnameFree(t *testing.T) {
	hv := &hostView{
		...
		Status: &hostStatus{
			...
			Hostname: "SECRET-HOSTNAME-MUST-NOT-LEAK", CycleStartUtc: "2026-06-10T15:44:32Z",
			OverallStatus: "fail",
		},
	}
...
func TestFetchRegistration(t *testing.T) {
	...
	full := serve(http.StatusOK, `{"poolId":"lab","gating":{"failuresBeforeAlert":5,"successesBeforeRearm":4,"quorum":{"healthyThreshold":0.75,"degradedAfterSeconds":600}}}`)
	defer full.Close()
	if pid, _, g, _, _, err := fetchRegistration(client, full.URL); err != nil || pid != "lab" || g == nil ||
		g.FailuresBeforeAlert != 5 || g.SuccessesBeforeRearm != 4 || g.HealthyThreshold != 0.75 || g.DegradedAfter != 600*time.Second {
...
func TestHandleGoCycle(t *testing.T) {
	...
	// (c) current-cycle fast path (no t): 302 to the LIVE base + current folder.
	{
		...
		s.hosts[hid] = &hostView{HostId: hid, BaseURL: "http://10.0.0.5:8080",
			Status: &hostStatus{CycleStartUtc: "c1", CycleFolderUrl: folder, StartedAt: "2026-06-12T10:48:00Z"}}
		w := get(s, "host="+hid)
...
func TestHandleGoHost(t *testing.T) {
	...
	// always lands on the status page, never a cycle folder (the /go/cycle distinction).
	{
		s := newPoolState("default", 8080)
		s.hosts[hid] = &hostView{HostId: hid, BaseURL: "http://10.0.0.5:8080/",
			Status: &hostStatus{CycleStartUtc: "c1", CycleFolderUrl: "status/log/000001." + hid + "/"}}
		w := get(s, "host="+hid)
