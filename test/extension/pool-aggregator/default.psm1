...
.SYNOPSIS
    ...
    'pool-control'), or '' when the pool knows none.

.DESCRIPTION
    A host that needs the stash service, or pool control, usually does not run
    ...
    record back. The aggregator lives in the caching-proxy VM, so knowing the
    ...
    caching proxy gets '' and falls back to whatever its caller does without a
    ...
    minted by the caching proxy's own CA, which a harness host has no trust store
...
.PARAMETER Area
    Extension area slug as registered/announced: 'stash-service', 'pool-control'.
...
function Get-PoolExtensionHost {
    ...
    if (-not (Get-Command Get-PoolAggregatorSeedUrl -ErrorAction SilentlyContinue)) {
        $cachingProxy = Join-Path (Split-Path -Parent (Split-Path -Parent $PSScriptRoot)) 'modules' |
            Join-Path -ChildPath 'Test.CachingProxy.psm1'
        if (Test-Path -LiteralPath $cachingProxy) {
            Import-Module $cachingProxy -Global -Force -DisableNameChecking -Verbose:$false
        }
    }
    if (-not (Get-Command Get-PoolAggregatorSeedUrl -ErrorAction SilentlyContinue)) {
        Write-Verbose "Get-PoolExtensionHost: Test.CachingProxy not loadable; cannot resolve the aggregator."
        return ''
    }
    $base = Get-PoolAggregatorSeedUrl
    if ([string]::IsNullOrWhiteSpace($base)) {
        Write-Verbose "Get-PoolExtensionHost: no caching-proxy address known, so no aggregator to ask."
        return ''
    }
...
.DESCRIPTION
    Split out so address RESOLUTION (which needs a caching proxy, a config file,
...
Export-ModuleMember -Function Get-PoolAggregatorManifest, Get-PoolExtensionHost, Get-PoolExtensionHostFrom
