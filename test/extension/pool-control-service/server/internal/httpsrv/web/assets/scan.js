...
(function () {
  ...
  const chrome = Y.initChrome({ refresh: function () { load(); } });
  ...
  function validate(raw) {
    ...
    const hosts = bits <= 30 ? size - 2 : size;
    return { ok: true, message: 'Scans ' + hosts.toLocaleString() + (hosts === 1 ? ' address.' : ' addresses.') };
  }

  function syncField() {
    const v = validate($('cidr').value);
    const hint = $('cidr-hint');
  ...
  function renderProgress(scan) {
    ...
    const what = scan.trigger === 'sweep' ? 'Periodic sweep' : 'Scan';
    if (scan.running) {
      box.className = '';
      box.textContent = '';
  ...
  function renderHosts(hosts) {
    ...
    for (const h of hosts) {
      const forget = Y.el('button', { text: 'Forget' });
      forget.addEventListener('click', async function () {
        if (!confirm('Stop monitoring ' + h.address + '?\n\nThe next scan of that network will find it again if it is still there.')) return;
        forget.disabled = true;
        try {
          Y.clearNotice();
          await Y.mutate('/api/scan/forget', { method: 'POST', body: { key: h.hostId || h.address } });
          await load();
        } catch (e) {
      ...
      // The address links to the host's own status service, which for a host
      // that registered with nobody is the only way to reach it. No base means
      // the prober did not say which port answered, and plain text beats a
      // link that cannot work.
      const where = h.baseUrl
  ...
  async function poll() {
    ...
    try {
      for (;;) {
        ...
        if (!data || !data.scan || !data.scan.running) return;
  ...
  async function startScan() {
    ...
    $('scan').disabled = true;
    try {
      const res = await Y.mutate('/api/scan', { method: 'POST', body: { cidr: cidr } });
  ...
  document.addEventListener('DOMContentLoaded', async function () {
    const data = await load();
    // A sweep may already be walking the network when the page opens; follow it
    // rather than showing a frozen snapshot of someone else's scan.
