...
func BenchmarkGlobalizationLookup(b *testing.B) {
	...
	if value := catalog.Render("stash.stashes_yuruna_stash", nil, locale); value == "stash.stashes_yuruna_stash" || len(catalog.MissingKeys()) != 0 {
		b.Fatal("benchmark requires the delivered locale catalog")
	}
	b.ReportAllocs()
	b.ResetTimer()
	for index := 0; index < b.N; index++ {
		if catalog.Render("stash.stashes_yuruna_stash", nil, locale) == "" {
