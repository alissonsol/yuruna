...
// recorder is a fake aggregator that captures every /announce body and can fail
// the first N requests (the boot catch-up scenario).
...
func TestEnabledRequiresURLHostIDAndInterval(t *testing.T) {
	cases := []struct {
		...
		{"nothing configured", New("", "", "stash-service", 80, 0), false},
	}
	for _, c := range cases {
...
func TestDisabledBeaconRunReturnsImmediately(t *testing.T) {
	b := New("", "", "download-agent-service", 80, time.Minute)
	done := make(chan struct{})
	go func() { b.Run(context.Background()); close(done) }()
	select {
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("a disabled beacon must not block")
	}
}
...
// The steady-state loop re-announces every Interval so the aggregator's announce
// TTL never expires while the service lives.
...
// afterwards the loop runs at Interval. Covers the whole-lab-reboot ordering
// where a service VM is up before the aggregator.
...
func TestHelloRetryDoublesToACeiling(t *testing.T) {
	got := firstHelloRetry
	for _, want := range []time.Duration{20 * time.Second, 40 * time.Second, maxHelloRetry, maxHelloRetry} {
		got = nextHelloRetry(got)
		if got != want {
...
// A period shorter than the standard catch-up wait pulls the first retry down
// with it: retrying slower than the steady-state cadence would be absurd.
func TestInitialHelloRetryNeverExceedsTheInterval(t *testing.T) {
	if got := initialHelloRetry(time.Hour); got != firstHelloRetry {
		t.Errorf("initialHelloRetry(1h) = %s, want %s", got, firstHelloRetry)
	}
	if got := initialHelloRetry(time.Second); got != time.Second {
		t.Errorf("initialHelloRetry(1s) = %s, want 1s", got)
	}
	if got := initialHelloRetry(0); got != firstHelloRetry {
		t.Errorf("initialHelloRetry(0) = %s, want %s", got, firstHelloRetry)
	}
}

// An https-configured beacon downgrades to plain http when the aggregator has no
// TLS leaf (transport-level failure), mirroring the host-side notifier's
...
func TestAnnouncesReuseTheKeepAliveConnection(t *testing.T) {
	var conns atomic.Int64
	srv := httptest.NewUnstartedServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		_, _ = w.Write([]byte(`{"ok":true,"note":"the aggregator answers with a body"}`))
	}))
	...
	b := New(srv.URL, "host-a", "download-agent-service", 80, time.Minute)
	for i := 0; i < 5; i++ {
		if err := b.announce(context.Background(), true); err != nil {
			t.Fatalf("announce %d failed: %v", i, err)
		}
	}
	// A body left unread makes the transport drop the connection, so every
	// announce would open a new one -- on a beacon that runs for the life of the
	// lab, against an aggregator that is also serving the dashboard.
	if got := conns.Load(); got != 1 {
		t.Fatalf("5 announces opened %d connections, want 1", got)
	}
}
