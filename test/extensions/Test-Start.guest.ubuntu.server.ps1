...
<#
.SYNOPSIS
    Drives the Ubuntu Server guest through initial boot via a JSON sequence.

.DESCRIPTION
    Reads the interaction sequence from ../sequences/Test-Start.guest.ubuntu.server.json
    and executes each step (keystrokes, delays, waits) against the VM.

    Identical wrapper pattern to Test-Start.guest.ubuntu.desktop.ps1 — the JSON
    file holds all the guest-specific behaviour. To customize the boot automation,
    edit the JSON file, not this script.
