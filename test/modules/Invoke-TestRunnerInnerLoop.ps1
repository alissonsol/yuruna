...
.SYNOPSIS
    ...
    cycle flow, config schema, notifications, and YURUNA_CACHING_PROXY_SERVICE_IP;
...
param(
    ...
    # <RepoRoot>/project from repositories.projectUrl). Used by Invoke-TestProject.ps1,
...
# Invoke-TestRunnerInnerLoop.ps1 by hand).
...
# Planner, Test.CachingProxyService, Test.Perf, Test.HostIO, Test.Capability,
...
# Write-UncommittedChangesWarning), and Assert-CachingProxyServiceStillReachable
# (per-step caching-proxy-service reachability probe).
...
# detect a single step that has exceeded testCycle.stepTimeoutSeconds.
...
# Test.SequencePlanner, Test.CachingProxyService, Test.Perf, Test.HostIO,
...
# (served at /runtime/host.registration.json) for the multi-host pool-aggregator service.
...
# Write-HostRegistrationRecord. No-op when this host runs no stash service.
...
# the per-step gate in Test.SequenceEngine.psm1 honors it too. But if a prior
...
# parent's to sweep (the outer's boot recovery, or Invoke-TestProject's PreSpawn
...
# Resolve-CachingProxyServiceEndpoint (Test.CachingProxyService.psm1). Acceptance is
...
# destroy $env:YURUNA_CACHING_PROXY_SERVICE_IP for downstream code. With no
...
if ($envCacheIp -or $configCacheIp) {
    ...
    # $env:YURUNA_CACHING_PROXY_SERVICE_IP. A reachable config IP therefore
...
# Proxy-cache detection lives in Test.CachingProxyService.psm1 so Start-StatusService
# shares the same probe -- console banner here and the status-page banner
# (via $env:YURUNA_RUNTIME_DIR/caching-proxy-service.txt) stay in lockstep with the
...
if ($cachingProxyUrl) {
    ...
    if ($isExternal) {
        ...
        # not the cache VM's real IP. Get-CachingProxyServiceVmIp reads the
        # yuruna-caching-proxy-service state file for the real IP so forwarders
        # tunnel to squid rather than looping back. On Windows the URL
        # already carries the cache VM IP.
        $portMapIp = Get-CachingProxyServiceVmIp
    ...
    if ($mapOk) {
        $dashboardUrl   = "http://${bestIp}:3000/d/yuruna-squid/caching-proxy-service-yuruna?orgId=1&from=now-2h&to=now&timezone=browser&refresh=1m"
        $esc            = [char]27
...
# enabled / -NoStatusService / port handling identical across the entry-point trio.
$null = Start-YurunaStatusServiceIfEnabled -Config $Config -StartScript $startScript -NoStatusService:$NoStatusService

# NOTE: the config service (mTLS NAS-credential endpoint) is NOT started here.
# It is a companion of the CACHING PROXY (it serves NAS creds to the caching-proxy-service
# / stash-service VMs a host created), so its lifecycle belongs to Start-CachingProxyServiceVM.ps1
...
$CycleStartUtc              = $cycleState.CycleStartUtc
...
if (-not $OverallPassed -and $FailedGuest) {
    ...
    if ($AlertArmed -and $ConsecutiveFailures -ge $FailuresBeforeAlert) {
        ...
        # Disarm so a stopOnFailure stream (this block fires only when
        # stopOnFailure broke before the in-cycle inline handler could
