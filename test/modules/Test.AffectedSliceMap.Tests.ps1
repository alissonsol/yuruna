...
Describe 'affected-slice map generation' {
    ...
    It 'does not count compiled table keys as protocol consumers but scans adjacent application code' {
        $fixture = New-SliceMapFixture
        $extra = Join-Path $fixture.Root 'src/catalog-runtime.js'
        [IO.File]::WriteAllText($extra, "// >>> yuruna-i18n embedded block -- generated by tools/Invoke-CatalogEmbed.ps1`nvar table = { 'sample.ready': 'Ready' };`n// <<< yuruna-i18n embedded block`n")
        $run = Invoke-FixtureMap -Fixture $fixture -Update
        Assert-Equal -Expected 0 -Actual $run.Code -Because $run.Output
        [IO.File]::AppendAllText($extra, "if (wireCode === 'sample.ready') { act(); }`n")
        $run = Invoke-FixtureMap -Fixture $fixture
        Assert-Equal -Expected 1 -Actual $run.Code -Because 'an undeclared application consumer beside a generated table escaped discovery'
        Assert-Match -Pattern 'unclassified source-only code literal' -Actual $run.Output
    }

    It 'requires implemented catalog boundaries to name their owner and verification suite' {
        $fixture = New-SliceMapFixture
        $authority = Read-FixtureJson $fixture.Manifest
        $row = $authority.rows[0]
        $row.kind = 'catalog-boundary'
        $row.status = 'implemented'
        $row.owner = 'G2-07'
        $row | Add-Member -NotePropertyName verification -NotePropertyValue 'src/boundary.Tests.ps1'
        Write-FixtureJson -Path $fixture.Manifest -Value $authority
        $run = Invoke-FixtureMap -Fixture $fixture
        Assert-Equal -Expected 1 -Actual $run.Code
        Assert-Match -Pattern 'no existing verification suite' -Actual $run.Output
        [IO.File]::WriteAllText((Join-Path $fixture.Root 'src/boundary.Tests.ps1'), '# Disposable verification-path fixture; not release evidence.')
        $run = Invoke-FixtureMap -Fixture $fixture -Update
        Assert-Equal -Expected 0 -Actual $run.Code -Because $run.Output
        $map = Read-FixtureJson $fixture.Output
        Assert-Equal -Expected 1 -Actual $map.summary.catalogBoundaryCount
        $row.owner = 'unassigned'
        Write-FixtureJson -Path $fixture.Manifest -Value $authority
        $run = Invoke-FixtureMap -Fixture $fixture
        Assert-Equal -Expected 1 -Actual $run.Code
        Assert-Match -Pattern 'no named G2 owner' -Actual $run.Output
    }
}
