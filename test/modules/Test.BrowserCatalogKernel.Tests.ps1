...
.TAGS yuruna test globalization catalog browser pester
...
<#
.SYNOPSIS
    Hold the browser catalog kernel to a single source, and hold every
...
.DESCRIPTION
    ...
    The third is independence from the engine. The kernel is rendered by a
    browser with Intl and String.prototype.normalize removed and with
    toLocaleString made locale-blind, so a formatter that quietly delegated to
    the engine would fail here. That delegation is exactly what the agreement
    cannot survive: ICU data differs between browsers and moves between
    releases, so a number that came from it would stop matching the same
    number written by PowerShell and Go.
...
Describe 'every runtime writes a value the same way' {
    ...
    It 'renders the shared fixture identically without the engine formatters' {
        if (-not $script:Chrome) {
            Set-ItResult -Skipped -Because 'no Chrome or Chromium on this host to render with'
            return
        }

        # Take away the engine's own formatters and make toLocaleString answer
        # in the browser's locale whatever it is asked for. A kernel that leaned
        # on any of these would render whatever ICU tables that build happens to
        # carry, and the runtimes would disagree on the same value.
        $preamble = @'
(function () {
...
"@
        ...
        Assert-NoFinding $findings 'a browser runtime disagrees with the shared formatting contract'
    }
}

Describe 'the kernel renders a page without the engine formatters' {
...
(function () {
  ...
  // Formatting data alone must not make a locale selectable. The tag is
  // synthetic so the check keeps testing that rule as locales are enabled.
  window.YurunaLocaleData['xx-XX'] = {group: ',', decimal: '.', groupSize: 3, plural: 'one-if-1', direction: 'ltr'};
  rows.push('unenabled=' + Y.resolve('xx-XX'));
  rows.push('enabled=' + Y.resolve('pt-BR'));
  document.getElementById('out').textContent = rows.join('\n');
}());
</script></body></html>
'@
        ...
        Assert-True ($text.Contains('unenabled=en-US')) `
            'a locale became selectable merely because localeData carries its formatting'
        Assert-True ($text.Contains('enabled=pt-BR')) `
            'a supported locale is not selectable'
    }

    It 'looks up, pluralizes and falls back with the engine formatters gone' {
        if (-not $script:Chrome) {
            Set-ItResult -Skipped -Because 'no Chrome or Chromium on this host to render with'
            return
        }
...
(function () {
  ...
  check('an alias resolves to its enabled target', Y.resolve('pt'), 'pt-BR');
  window.YurunaLocaleData['xx-XX'] = {group: ',', decimal: '.', groupSize: 3, plural: 'one-if-1', direction: 'ltr'};
  check('formatting data alone is not selectable', Y.resolve('xx-XX'), 'en-US');
  ...
  check('Intl really is gone', typeof window.Intl, 'undefined');
  document.getElementById('out').textContent = rows.join('\n');
}());
</script></body></html>
"@
        ...
        Assert-NoFinding $findings 'the kernel behaves differently once the engine formatters are gone'
    }
}
