    ...
    It 'names every Go file on disk, and none that is not' {
        ...
        foreach ($loop in $loops) {
            ...
            foreach ($name in $sources) {
                ...
                # Raw services stage one self-contained generated registry. A separate
                # generated Data constant is redundant only when its exact payload is
                # already in that registry and it contains no executable declarations.
                if ($name -match '^internal/catalog/(?!registry\.go$)[^/]+\.go$' -and
                    $listed -contains 'internal/catalog/registry.go') {
                    $catalogText = [IO.File]::ReadAllText((Join-Path $dir $name))
                    $constant = [regex]::Match($catalogText, '(?s)\A// Generated by tools/Invoke-CatalogCompile\.ps1.*?\npackage catalog\s+//[^`]+const Data\w+ = (`.*`)\s*\z')
                    Assert-True $constant.Success "$area : $name is not a standalone generated catalog constant"
                    $registryText = [IO.File]::ReadAllText((Join-Path $dir 'internal/catalog/registry.go'))
                    Assert-True ($registryText.Contains($constant.Groups[1].Value)) "$area : the staged registry does not contain the exact $name payload"
                    continue
                }
