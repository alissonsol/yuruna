...
function Invoke-NewVM {
    ...
    $scriptPath = Join-Path $RepoRoot (Join-Path $hostFolder (Join-Path $GuestKey "New-VM.ps1"))
...
function Remove-UtmTestVM {
    ...
    if ($PSCmdlet.ShouldProcess($VMName, 'Remove VM')) {
        ...
        if (Test-Path $utmBundle) {
            # test/modules/Test.New-VM.psm1 → host/macos.utm/VM.common.psm1
            $virtualHelper = Join-Path $PSScriptRoot "../../host/macos.utm/VM.common.psm1"
