...
.TAGS yuruna test pool aggregator seed url probe pester
...
<#
.SYNOPSIS
    Pins the probe-and-discard contract of Get-PoolAggregatorSeedUrl, the
    aggregator address every pool consumer resolves and guest seeds carry.
.DESCRIPTION
    A host can hold three claims about where its caching proxy is, and each
    ...
    Run: pwsh -NoProfile -File test/modules/Test.PoolAggregatorSeedUrl.Tests.ps1
#>
...
Describe 'Get-PoolAggregatorSeedUrl believes a claim only once it answers' {
    BeforeEach {
        $env:YURUNA_CACHING_PROXY_IP = ''
        Mock Save-CachingProxyState -ModuleName Test.CachingProxy { 'state-path' }
    }
    AfterEach {
        $env:YURUNA_CACHING_PROXY_IP = ''
    ...
    Context 'a claim that does not answer is passed over' {
        It 'takes the live config address over a stored and an exported one that are both dead' {
            ...
            Mock Test-TcpPortReachable   -ModuleName Test.CachingProxy { return ($TargetHost -eq '192.0.2.12') }
            $env:YURUNA_CACHING_PROXY_IP = '192.0.2.11'
            Use-ConfigProxyAddress '192.0.2.12'
            Assert-Equal -Expected 'https://192.0.2.12:9400' -Actual (Get-PoolAggregatorSeedUrl) `
        ...
        It 'repairs the stored address it disproved, so the next call pays no probe for it' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.13' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { return ($TargetHost -eq '192.0.2.14') }
            Use-ConfigProxyAddress '192.0.2.14'
            [void](Get-PoolAggregatorSeedUrl)
            Assert-MockCalled Save-CachingProxyState -ModuleName Test.CachingProxy -Scope It -Times 1 -Exactly `
                -ParameterFilter { $IpAddress -eq '192.0.2.14' }
        }
        It 'skips a claim carrying a quote and lets the next one resolve' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = "192.0.2.15'" } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { return ($TargetHost -eq '192.0.2.16') }
            Use-ConfigProxyAddress '192.0.2.16'
            Assert-Equal -Expected 'https://192.0.2.16:9400' -Actual (Get-PoolAggregatorSeedUrl) `
    ...
    Context 'the order still ranks claims that do answer' {
        It 'keeps the stored address when it answers, even though a config address also does' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.20' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { $true }
            Use-ConfigProxyAddress '192.0.2.21'
            Assert-Equal -Expected 'https://192.0.2.20:9400' -Actual (Get-PoolAggregatorSeedUrl) `
                -Because 'a host that runs its own proxy must keep using it'
        }
        It 'writes nothing when the stored address is the one that won' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.22' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { $true }
            Use-ConfigProxyAddress '192.0.2.23'
            [void](Get-PoolAggregatorSeedUrl)
            Assert-MockCalled Save-CachingProxyState -ModuleName Test.CachingProxy -Scope It -Times 0 -Exactly
        }
    }

    Context 'the probe proves the address the answer names' {
        It 'probes the aggregator port rather than the proxy port' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { $true }
            Use-ConfigProxyAddress '192.0.2.30'
            [void](Get-PoolAggregatorSeedUrl)
            Assert-MockCalled Test-TcpPortReachable -ModuleName Test.CachingProxy -Scope It -Times 1 -Exactly `
    ...
    Context 'a host that stores no address of its own' {
        It 'resolves from the config key without starting to claim ownership' {
            ...
            Assert-Equal -Expected 'https://192.0.2.40:9400' -Actual (Get-PoolAggregatorSeedUrl)
    ...
    Context 'nothing answers' {
        It 'returns empty so the consuming service leaves its features off' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.50' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { $false }
            Use-ConfigProxyAddress '192.0.2.51'
            Assert-Equal -Expected '' -Actual (Get-PoolAggregatorSeedUrl)
        }
        It 'leaves the stored address alone when there is nothing verified to replace it with' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.52' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { $false }
        ...
        It 'returns empty without probing when no claim exists at all' {
            ...
            Assert-Equal -Expected '' -Actual (Get-PoolAggregatorSeedUrl)
            Assert-MockCalled Test-TcpPortReachable -ModuleName Test.CachingProxy -Scope It -Times 0 -Exactly
        }
    }

    Context 'a preview resolves but does not write' {
        It 'still returns the address so the preview can name it' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.60' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { return ($TargetHost -eq '192.0.2.61') }
            Use-ConfigProxyAddress '192.0.2.61'
            Assert-Equal -Expected 'https://192.0.2.61:9400' -Actual (Get-PoolAggregatorSeedUrl -WhatIf)
        }
        It 'repairs nothing under -WhatIf' {
            Mock Read-CachingProxyState -ModuleName Test.CachingProxy { @{ ipAddress = '192.0.2.62' } }
            Mock Test-TcpPortReachable  -ModuleName Test.CachingProxy { return ($TargetHost -eq '192.0.2.63') }
            Use-ConfigProxyAddress '192.0.2.63'
            [void](Get-PoolAggregatorSeedUrl -WhatIf)
    ...
    Context 'the cost is paid once per process' {
        It 'answers a repeat call from the memo instead of re-probing' {
            ...
            $first  = Get-PoolAggregatorSeedUrl
            $second = Get-PoolAggregatorSeedUrl
