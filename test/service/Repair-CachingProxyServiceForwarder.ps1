...
.EXAMPLE
    pwsh test/service/Repair-CachingProxyServiceForwarder.ps1

.EXAMPLE
    # State file has the wrong IP; you already know the right one:
    pwsh test/service/Repair-CachingProxyServiceForwarder.ps1 -CacheIp 192.168.7.150
#>
...
if ($CacheIp) {
    ...
    if (-not $foundUrl) {
        ...
        Write-Error "  If the VM is stopped/missing, rebuild with: pwsh test/service/Start-CachingProxyServiceVM.ps1"
