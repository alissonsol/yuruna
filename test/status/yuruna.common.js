...
(function() {
  ...
  // Read routes are unaffected. No proof -> loopback only.
  (function() {
    try {
  ...
  // and absolute http(s), reject everything else. Returns '' for anything not
  // provably safe so callers fall back to rendering plain text.
  function safeUrl(v) {
  ...
  // <bdi> does this where there is an element to use. The explicit isolate
  // characters do it everywhere else, including in a plain-text context that
  ...
  // The same isolation as an element, for a surface that builds DOM. The
  // element carries the isolation itself, so the text inside it keeps only
  // its letters; no stylesheet here sets unicode-bidi on bdi, which is the
  // one thing that would neutralize it.
  function bidiElement(value) {
    var el = document.createElement('bdi');
    el.textContent = withoutBidiControls(value);
    return el;
  }
        ...
        if (type === 'array') {
          var renderArrayChildren = function() {
            ...
            value.forEach(function(item, idx) {
              ...
              // Find the direct-child .tree-row without the :scope selector. The row is the
              // node's own first element child; scan direct children defensively.
              var childRow = null;
    ...
    function makeCacheIpProbeDriver(markEl) {
      ...
      return function(ip, trigger) {
        ...
        debounceTimer = setTimeout(function() {
          ...
          // operator keeps typing. Bound the fetch so a black-holed cache
          // resolves to a failed mark instead of an indefinite pending one.
          var ctrl     = new AbortController();
          var timedOut = false;
          var settled  = false;
          // The timer reports the outcome itself rather than relying on the
          // abort to make the request fail, so the mark lands on a stated
          // result instead of waiting on whatever the canceled request does.
    ...
    function buildCustomSelect(opts) {
      ...
      for (var i = 0; i < opts.options.length; i++) {
        (function (item, idx) {
          ...
          // pointerdown unifies mouse + touch + pen, so one handler covers every input
          // device and fires once per selection.
      ...
      // One pointerdown on the document, so an outside tap closes the menu.
      var docCloseEvents = ['pointerdown'];
      function open() {
