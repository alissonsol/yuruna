...
# Probe :3128 on the host's loopback — this is where the forwarder binds
# (0.0.0.0:3128, so 127.0.0.1 and 192.168.64.1 both work). 127.0.0.1 is
# the most portable check and doesn't depend on the VZ bridge being
...
# Fetch the squid-cache CA on the host so it can be base64-embedded in the
# autoinstall seed. See guest.ubuntu.desktop/New-VM.ps1 for the same block
# with full context; short version: guests on VZ shared-NAT cannot reach
# the cache VM directly, but this script runs on the host which can. Any
# failure (no cache-ip.txt, HTTP error, empty response) leaves $CaCertBase64
