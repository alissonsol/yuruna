...
- **Password**: `password` — for console login via `vmconnect localhost squid-cache`.
  Password expiry is disabled, so repeated console or `ssh -o PreferredAuthentications=password`
  sessions keep working without an interactive reset.
