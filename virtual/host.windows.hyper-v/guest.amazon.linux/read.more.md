...
- Run `/automation/fetch-and-execute.sh virtual/guest.amazon.linux/amazon.linux.update.sh` (cloud-init's `runcmd` pre-downloads `fetch-and-execute.sh` into `/automation/` — the workload script itself is fetched from GitHub on demand). This installs the GUI and tools.
